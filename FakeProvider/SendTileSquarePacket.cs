...
namespace FakeProvider
{
    class SendTileSquarePacket
    {
            ...
            int Width, int Height, int X, int Y, int TileChangeType = 0)
        {
            ...
            if (clients.Count > 0)
				FakeProviderPlugin.SendTo(clients, Generate(X, Y, Width, Height, TileChangeType, -1));
			foreach (RemoteClient client in personalClients)
				FakeProviderPlugin.SendTo(new List<RemoteClient>() { client }, Generate(X, Y, Width, Height, TileChangeType, client.Id));
		}

        #endregion
        #region Generate

        private static byte[] Generate(int X, int Y, int Width, int Height, int TileChangeType, int PlayerIndex = -1)
        {
			...
			using (BinaryWriter bw = new BinaryWriter(ms))
			{
				...
				if (PlayerIndex >= 0)
					WriteTiles(PlayerIndex, bw, X, Y, Width, Height, TileChangeType);
				else
		...
		/// <param name="number3">Y</param>
		/// <param name="number5">TileChangeType</param>
		private static void WriteTiles(int playerIndex, BinaryWriter binaryWriter,
            int number, int number2, int number3, int number4, int number5 = 0)
		{
			int sx = number;
			int sy = (int)number2;
			...
			binaryWriter.Write((byte)height);
			binaryWriter.Write((byte)number5);
			ITile[,] tiles = FakeProviderAPI.ApplyPersonal(playerIndex, sx, sy, width, height);
